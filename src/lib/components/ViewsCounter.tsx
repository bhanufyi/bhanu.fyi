...
export const ViewsCounter = ({
  ...
  useEffect(() => {
    async function fetchPageViews() {
      const { data, error, type } = await isomorphicFetchPageViews(slug, {
        disabled,
      });
    ...
    case 69:
      return (
        <>
          ...
          <ViewCount>{view_count.toLocaleString()}</ViewCount> times. Nice.
        </>
      );
    case 420:
      return (
        <>
          This {page_type} has been viewed{" "}
          <ViewCount>{view_count.toLocaleString()}</ViewCount> times. Hehe.
        </>
      );
    ...
    default: {
      if (view_count > 100) {
        return (
          <>
            This {page_type} has been viewed{" "}
            <ViewCount>{view_count.toLocaleString()}</ViewCount> times. Wow.
          </>
        );
        ...
        return (
          <>
            This {page_type} has been viewed{" "}
            <ViewCount>{view_count.toLocaleString()}</ViewCount> times. Holy
            crap. 🤯
          </>
        );
      ...
      return (
        <>
          This {page_type} has been viewed{" "}
          <ViewCount>{view_count.toLocaleString()}</ViewCount> times
        </>
      );
    }
  }
