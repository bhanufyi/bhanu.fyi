...
function* colorSchemeGenerator(): Generator<
  COLOR_SCHEMES,
  never,
...
> {
  ...
  while (true) {
    const override: COLOR_SCHEMES | undefined = yield COLOR_SCHEME[
      current
    ] as COLOR_SCHEMES;
...
export const ColorSchemeToggle = () => {
  ...
  useEffect(() => {
    ...
    const colorSchemeDarkMediaQuery = window.matchMedia(
    ...
    colorSchemeDarkMediaQuery.addEventListener(
      "change",
      colorSchemeChangeListener,
    );

    return () =>
      colorSchemeDarkMediaQuery.removeEventListener(
        "change",
        colorSchemeChangeListener,
      );
...
const ToggleIcon = ({
  ...
  switch (colorScheme) {
    case "light":
      return (
        <LuSun
          aria-label="Currently using light mode"
          title="Currently using light mode"
        />
      );
    case "dark":
      return (
        <IoMdMoon
          aria-label="Currently using dark mode"
          title="Currently using dark mode"
        />
      );
    case "system":
      return (
